...
def ether(data):
    ...
    return [get_mac_addr(dest_mac), get_mac_addr(src_mac), hex(proto), data[14:]]
...
def udp(data):
    ...
    return [data[0], data[1], data[2], hex(data[3]), maindata[8:]]


def arp(data):
    arp_detailed = unpack('!HHBBH6s4s6s4s', data[:28])
    return [arp_detailed[0],
            hex(arp_detailed[1]),
            arp_detailed[2],
            arp_detailed[3],
            arp_detailed[4],
